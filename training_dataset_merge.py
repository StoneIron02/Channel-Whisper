...
for i in range(count, 101):
    ...
    audio_length = get_audio_length(file_prefix + i_str + ".wav")
    data.append(audio_length)

    with open(file_prefix + i_str + ".txt", 'r', encoding='utf-8') as file:
        text = file.read()
        data.append(text)
    ...
    for model in models:
        ...
        audio = whisper.load_audio(file_prefix + i_str + ".wav")
    ...
    with open('data_t_m.csv', 'a', newline='') as file:
        ...
        try:
            ...
            pass
    with open('recent_number_t_m.txt', 'w') as file:
        file.write(str(i))
